...
public class Ball extends PhysicsObject {
		...
		for(PhysicsObject p: super.getHandler().getObjects()){
			if(p.getId() == ID.Ball){
				...
				if(!b.getColor().equals(getColor())){
					Manifold m = new Manifold(this, b);
	...
	public double getDistance(Ball b){
		double r = radius + b.getRadius();
		r *= r;
		
		Vector2D normal = Vector2D.subtractVectors(b.getPosition(), this.getPosition());
		
		if(normal.getMagSq() > r){
	...
	public boolean isColliding(Manifold m){
		Ball a = m.getA();
		Ball b = m.getB();
		
		Vector2D n = Vector2D.subtractVectors(b.getPosition(), a.getPosition());
		...
		if(d != 0){
			m.setPen(Math.abs(r-d));
			
			m.setNorm(n.divideRet(d));
